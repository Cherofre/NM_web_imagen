...
## Handoff Notes
- Start here: Task 3 in `docs/superpowers/plans/2026-07-15-final-review-remediation.md`; begin with a failing test that saves the same data URL reference twice and observes two different UUID URLs/files.
- Do not redo: remediation Tasks 1-2, fixes through `4a4499e`, Tasks 1-5 before the final review, or the July 11 full verification merely to prove the rejected candidate again. New code changes will receive targeted tests first and the full matrix at the end.
- Verify next: identical session/turn/reference IDs plus identical bytes reuse one stable `/outputs/session_refs/...` URL/file; different bytes remain distinct; old UUID URLs stay unchanged.
