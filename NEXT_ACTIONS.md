...
- [ ] Hard-refresh `http://127.0.0.1:14261/` and confirm the bottom Cancel button closes the reference-switch confirmation exactly like X without switching sessions or clearing references. Then reopen the base image, redraw the external background mask, keep `标准`, and run one low-cost single-image edit with a concrete target such as `把涂红的外部背景改成夜晚城市，人物保持不变`. Confirm the result row shows `遮罩引导`, no red marker remains, the requested region changes, and hair/dress/mist/lighting cross the selection boundary naturally without the former cut contour. Small protected-area drift is possible and must be judged rather than claimed away.
- [ ] Follow up the two reported mask usability defects: regenerate must carry the original runtime mask into the new turn (or explicitly block after refresh when the editable mask no longer exists), and the mask editor must calculate its initial/fit scale from the actual stage dimensions so the full image is visible before drawing. Do not package, sync G:, push, tag or release without a new explicit instruction.

## Handoff Notes
- Start here: hard-refresh `14261`, verify bottom Cancel dismisses the session-switch confirmation, then manually test the model-guided `标准` mask with one concrete background replacement. The upstream request sees the red selection directly and the saved result is not locally pasted or clipped.
...
- Verify next: confirm Cancel leaves both the active session and current references unchanged. Then confirm a `遮罩引导` result removes the red guide, changes the selected background, keeps the person recognizably stable, and blends the boundary without a cut contour. After that, implement and verify regenerate-mask reuse plus true fit-to-stage.
...
- Release boundary: local `main` contains v1.0.7 through `af4afc0`; v1.0.8 remains isolated on `codex/history-window-refactor-v1.0.8`. G: remains unchanged. Current portable backend PID `51720` runs on `14261`; the existing `14260` service is separate.
- Current Studio assets: `index-2X_EvAWd.js` and `index-DFqFlLMm.css`.
