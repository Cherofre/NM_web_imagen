...
- [x] Complete the initial page-local mask-regenerate and editor-fit follow-up: regenerate carries a detached runtime mask into the new turn, while the editor calculates initial/fit scale from actual stage dimensions with `ResizeObserver`. The page-local refresh limit recorded at this step is superseded by the persisted-Alpha item below. Full matrix: 156 Node, 242 Python, size/build/compile/diff/HTTP smoke green; `14261` served `index-Ddq3RQ_S.js` / `index-DAW6P8i1.css` from PID `27732`.
...
- [x] Run the full pre-merge review and correct the release/cache identity from 1.0.7 to 1.0.8. The branch is a clean 30-commit fast-forward from main; 160 Node, 244 Python, 33 release-cache, size/build/compile/diff/import/HTTP gates pass with zero remaining P0/P1 findings. Browser loopback access is policy-blocked and was not bypassed.
...
- Release boundary: local `main` contains v1.0.7 through `af4afc0`; v1.0.8 remains isolated on `codex/history-window-refactor-v1.0.8`. G: remains unchanged. Current portable backend PID `53128` runs on `14261`; the existing `14260` service remains separate at PID `56600`.
- Current Studio assets: `index-jekAejtM.js` and `index-B-pQphnX.css`.
