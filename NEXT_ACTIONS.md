...
## Handoff Notes
- Start here: Task 4 in `docs/superpowers/plans/2026-07-15-final-review-remediation.md`; begin with failing `sessionRevision.test.mjs` cases for startup three-way merge and conditional canonical reference adoption.
- Do not redo: remediation Tasks 1-3, fixes through `56e3bc0`, Tasks 1-5 before the final review, or the July 11 full verification merely to prove the rejected candidate again. New code changes will receive targeted tests first and the full matrix at the end.
- Verify next: startup preserves newer local edits and server-only sessions, baseline-aware deletions work in both directions, and a save adopts server `/outputs` reference fields only when the current src still equals the sent src.
