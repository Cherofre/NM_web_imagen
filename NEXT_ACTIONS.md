...
- [x] Add product commit `e5a2bc1`: keep official transparent-selected masks as `标准`, add a per-mask reverse-Alpha `兼容网关` option, restore the painted selection correctly when reopening, and verify it with 149/149 Node tests, size rules, warning-free build, 33/33 release-cache tests, diff check and live HTTP smoke. The unchanged backend retains its prior 239/239 Python baseline; `14261` serves `index-DRPQN9Wy.js` / `index-DFqFlLMm.css`.
...
- [ ] Refresh `http://127.0.0.1:14261/`, reopen the same background mask, choose `兼容网关`, and run one low-cost single-image edit with a concrete target such as `只把涂红的外部背景改成夜晚城市，人物的脸、头发、衣服、姿势和构图保持不变`. Then finish the pending history/preview interaction checks. Do not package, sync G:, push, tag or release without a new explicit instruction.

## Handoff Notes
- Start here: manually test `兼容网关` on `14261` using one image and a concrete background replacement target. No more code changes are needed before that signal.
- Do not redo: alpha-direction inspection, multipart inspection, LiteLLM route inspection, official-doc lookup, or the three base/review/result comparisons. The tool now exposes both plausible Alpha conventions without silently changing the official default.
- Verify next: compare selected versus protected-region drift. If compatibility mode succeeds, keep it as an explicit gateway workaround. If both modes fail, proceed to prompt hardening/targetless-prompt validation or design strict post-compositing. Then test Escape in the order `preview -> detail/origin -> history surface` and wheel/touchpad-scroll the compact history grid at a narrow width.
...
- Current Studio assets: `index-DRPQN9Wy.js` and `index-DFqFlLMm.css`.
- Known limits: automatic real-browser smoke is blocked by enterprise loopback policy; compatibility mode is attached only to the current in-memory mask; old records have no recoverable mask data; new records persist only a review composite, not the editable full mask; GPT Image mask boundaries are prompt guidance rather than a hard cutout; custom gateways may reverse or ignore edits multipart `mask` semantics.
