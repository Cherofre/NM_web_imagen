...
双击启动脚本时会自动打开带版本参数的地址，例如：

```text
http://127.0.0.1:7861/?v=1.1.4
```

这个版本参数用来避开浏览器缓存旧入口页。覆盖更新后重新双击启动脚本即可，不需要手动改文件名或清浏览器缓存。

## 分享到其他电脑（1.1.4）

- 发行包面向 Windows 10/11 x64。无需自行安装 Python、Node.js、Rust 或配置开发环境。
- 新电脑或无法联网安装环境时，选择 **离线 WebView2 安装版**；普通安装版在缺少 WebView2 时需要联网下载。
- 便携桌面版仍需系统已安装 WebView2 Runtime。请完整解压后运行，不要只复制 EXE，也不要直接从压缩软件里启动。
- 网页版内含便携 Python 和离线依赖，使用现代 Edge/Chrome 打开。把目录放在当前用户可写的位置，中文和空格路径均可使用。
- 安装依赖可以离线完成；实际生图仍需网络和有效的模型接口配置。
- 更新便携版请保留 `data`，更新网页版请保留 `config.local.json` 和 `outputs`。缺少运行环境或启动失败时请保留错误提示和日志，便于定位。

多参考图使用自动接口时会按顺序送到图片编辑接口，第一张为底图。第三方渠道的不同模型可能在编辑、参考图遵循和输出尺寸上存在差异；请求成功不代表模型一定遵循参考图。若某个模型忽略参考图，可先换用同渠道已验证可用的模型，并向渠道反馈模型名、编辑接口、参考图数量和时间。
...
- 超时 / 无限超时
- 最多 16 张参考图，第一张可作为局部编辑底图
- 局部遮罩编辑：画笔、橡皮、移动、粗细、撤销、重做、清空、全选和缩放

#### GPT 局部遮罩

1. 保持在 GPT Image 2 的“生图”模式，添加至少一张参考图。
2. 第一张参考图会显示“底图”标记，点击旁边的编辑按钮打开遮罩画布。
3. 用画笔涂出允许修改的区域，红色区域表示允许模型修改；橡皮用于恢复保护区域。
4. 点击“应用遮罩”后再正常提交。工具会把第一张底图和遮罩转换为同尺寸 PNG，并使用 `/v1/images/edits` 发送。

注意：

- 遮罩只作用于第一张底图，后续图片仍作为内容或风格参考。
- 提示词仍需明确描述要改成什么。遮罩是模型的编辑提示，边界不保证像素级完全贴合。
- 遮罩只保存在当前页面内存和已排队任务快照中，刷新页面后不会恢复。
- 使用遮罩时，接口类型只能选择 `auto` 或 `/v1/images/edits`；自定义兼容网关也必须支持 edits multipart 的 `image[]` 与 `mask` 字段。
