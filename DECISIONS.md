...
## Active Decisions
- 2026-07-21 — Status: active: Mask containment is now enforced locally, not entrusted to GPT or the compatibility gateway. For every strict masked edit, require a concrete transformation target before upstream, append preservation constraints, disable `enhance_prompt`, then use Pillow to composite the first base image back over every unpainted pixel before saving/history/response. Interpret standard and compatibility Alpha according to the active attachment so both preserve the same painted selection. Cap strict masks at 8,294,400 pixels and fail closed if decoding, ratio alignment or compositing fails. The model may still determine the painted content, and slight edge seams remain an explicit tradeoff. Commit: `ccc6fdc`.
- 2026-07-21 — Status: superseded: Keep official transparent-selected Alpha as the default `标准` mask encoding, and expose `兼容网关` as an explicit per-mask reverse-Alpha diagnostic/workaround. The 10:48 real comparison also failed and changed the protected person about 2.3 times more than the painted background, so Alpha switching remains available only as diagnosis and no longer owns containment; `ccc6fdc` supersedes it with strict local restoration.
