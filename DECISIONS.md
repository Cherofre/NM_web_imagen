...
## Active Decisions
- 2026-07-18 — Status: active: Product commit `976595c` is the current manually-reviewable Studio candidate. Result image actions are grouped by object relationship, internal filenames are hidden, shared dimensions move into the completion metadata row, menus dismiss without moving cards, and native page-image drag is blocked from the external-file drop workflow. No backend or persistence contract changed.
