...
## Active Decisions
- 2026-07-20 — Status: active: Keep `gpt-image-2` as the image model and expose `gpt-5.6` as the new-chat-model default/preset, while preserving existing saved chat model values. The header summarizes both as `生图 … · 聊天 …` so the two responsibilities are not confused. Reasoning defaults to `auto`, which omits the parameter and lets each upstream model choose its own default; the UI also accepts `max` for GPT-5.6 and keeps custom model input. Do not auto-probe `/models` at startup because shared/relay endpoints do not consistently implement that route; the explicit diagnostics action remains the compatibility check.
- 2026-07-20 — Status: active: On narrow screens, the engine switcher is content-width and left-aligned while header actions remain full-width and wrappable. This preserves the existing pill vocabulary without presenting an empty full-row capsule or changing the desktop hierarchy.
