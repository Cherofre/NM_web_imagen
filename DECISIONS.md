...
## 2026-09-23 - 附加提示词命名
- Status: active
- Decision: 用户在候选讨论后明确选定「附加提示词」作为输入框下方入口名；窄屏保留完整名称，弹窗标题同步。此入口仍包含固定提示词、负面提示词和画面文字，功能不变。
- Workflow: 纳入本地 1.1.4 四类包；当前不发布、不覆盖用户正在使用的安装版。

## 2026-09-23 - 配置弹窗独立滚动
- Status: active
- Decision: 保留原生聊天模型 select 和既有界面；修正四个直接子项的 Grid 行数，让中间表单滚动、标题与底部操作固定。修复纳入同一 1.1.4 的四类包。
- Evidence: 修复前 720px 高窗口中底部按钮 y=891.94；修复后带诊断结果的四种窗口尺寸均可选择聊天模型且按钮可见。

## 2026-09-17 - 1.1.4 桌面更新兜底与打包
- Status: active
- Decision: 用户要求将模型刷新修复纳入 1.1.4，并审查更新和 DSH 方案；重建现有四类包，不自动沿用此前 Release 发布授权。
- Decision: 签名 feed 不可用时，仅通过固定 GitHub API 发现公开正式版本并打开发布页手动升级；应用内安装继续要求原 Tauri 签名。原私钥不明确时不替换内置公钥、不绕过验签。
- Evidence: v1.1.3 latest Release 没有 latest.json/portable-latest.json，两个客户端地址均 404；原事件大小写和 contentLength 协议不匹配。历史下载失败缺完整日志，不宣称已复现其全部根因。

## 2026-09-17 - 模型列表刷新采用替换语义
- Status: active
- Decision: 成功读取的非空列表是本次刷新依据，不再合并历史模型；失效当前模型回退到首项，有效选择保留；失败/空响应沿用已有错误提示并保留表单。手动选择或输入模型仍使用合并逻辑。
- Reason: 原 mergeImageModelOptions 同时保留旧缓存和当前模型，导致渠道已移除的 YS 模型始终残留。
- Workflow: ledger-only，小范围前端修复与构建；不沿用历史发布授权，不替换用户正在使用的桌面程序。
...
- Evidence: 截图三次本地历史均为 edits + 4 张图；模型别名多图字节与顺序回归通过。未获取上游接收证据，不把效果问题误报为已修复的客户端漏图。
- Workflow: ledger-only；构建本地四类包并同步既有 G 盘干净分发目录，不沿用上轮 GitHub 发布授权。
- Compatibility: 以 Windows 10/11 x64 为发行边界；使用带 WebView2 的离线安装器降低新电脑依赖风险；真实异机/升级验证仍须单独验收。
