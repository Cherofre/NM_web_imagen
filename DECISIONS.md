...
## Active Decisions
- 2026-07-21 — Status: active: Masked turn regeneration is truthful and page-local. Store a detached runtime payload keyed by turn ID containing `maskFile`, compact `previewFile`, coverage and encoding, but never retain another full base image. On regenerate, download that turn's first reference, bind the payload to the new `File`, pass the attachment and existing review snapshot explicitly, and register the new turn for another reuse. If the payload disappeared after refresh or the base cannot load, stop with a redraw message; never silently fall back to an unmasked request. Every regenerate supplies either that turn's attachment or `null`, preventing the current composer mask from leaking across turns.
...
- 2026-07-21 — Status: active: Masking is a model-guidance workflow, not a local cut-and-paste guarantee. For every accepted mask request, keep concrete-target validation and disable `enhance_prompt`; construct the first upstream edit image by tinting the user's actual editable selection red at 58%, send the real Alpha mask alongside it, and prompt for a complete final image where red disappears and only an immediate boundary band may change for natural light/texture/mist/perspective continuity. Save the upstream output directly with `mask_guidance=visual-alpha`; never restore, clip, feather or paste result pixels locally. Keep the 8,294,400-pixel ceiling. This favors a truthful natural edit over false pixel-exact preservation, so small protected-area drift remains an explicit limitation. Commit: `8ff25c9`. Official basis: OpenAI's mask guidance says the non-covered area is edited, protected content may still change, and the prompt should describe the complete resulting image.
...
- 2026-07-20 — Status: active: Persist a lightweight mask-review composite, not the editable full-resolution mask. Applying a mask still keeps the base/mask PNG files only in the in-memory attachment and queued request, but also exports a longest-edge 384px WebP showing the base plus the editor's 52% red overlay. Store that optional `maskSnapshot` through the existing Studio session-reference normalization, capacity and canonical-URL flow. Show it as a labeled turn thumbnail and disable reference/mask-edit actions in its preview. This makes shape and position reviewable after reload without turning session JSON into a full mask archive; records created before this change cannot be reconstructed.
