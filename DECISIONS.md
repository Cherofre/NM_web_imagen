...
## Active Decisions
- 2026-07-21 — Status: active: The user authorized local integration after the full v1.0.8 review. Fast-forward `codex/history-window-refactor-v1.0.8` into local `main` through `cd3b1bd`, then keep the screenshot-driven Advanced Parameters deduplication as the small follow-up commit `fd3c83b`. This authorization covers local merge and local service restart only; it does not authorize push, package, G: synchronization, tag, pull request, GitHub Release or branch/worktree deletion. Manual browser acceptance remains a separate gate.
