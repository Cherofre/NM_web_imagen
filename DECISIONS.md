...
## Active Decisions
- 2026-07-20 — Status: active: The v1.0.8 implementation uses one `closed / quick / browser(detailId?)` history state. The compact popover shows the newest image-bearing batch, the full window is grid-only with four-cell batch collages, and history context replaces the grid inside the same modal. Esc/back restores the grid scroll position and closing restores focus to the fixed sidebar trigger. Existing history APIs, schemas, record-level favorites and delete semantics remain unchanged.
