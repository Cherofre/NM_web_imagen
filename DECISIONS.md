...
- 2026-07-20 — Status: active: At tablet/narrow-desktop widths, use a two-column header grid with engine tabs and configuration on the first control row and language/more utilities aligned compactly to the right below it. At phone widths, engine spans the row and configuration shares the next row with utilities; below 380px, all three groups stack. This replaces width-only trimming with structural reflow and preserves touch targets.
