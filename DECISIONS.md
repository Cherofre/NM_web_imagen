...
- 2026-07-18 — Status: active: Product commit `4d22573` is the locally verified UI implementation. Single results default to true-aspect contained previews; multiple results remain compact until expanded; result cards expose three text actions plus a details menu. Header clearing is explicit and dangerous, generation size/quality/count share one popover, conversation/composer share a 980px column, and the queue/radius system is restrained. This is UI-only: no backend/data protocol changed, and no package/G:/push/merge/release occurs before manual browser acceptance.
