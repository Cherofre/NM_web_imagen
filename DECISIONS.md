...
## Active Decisions
- 2026-07-21 — Status: active: Keep official transparent-selected Alpha as the default `标准` mask encoding, and expose `兼容网关` as an explicit per-mask reverse-Alpha diagnostic/workaround. The choice follows only the current in-memory mask, is visible through a compatibility badge, and must not become a silent global inversion or a persisted profile option. A real comparison decides the next step: compatibility success means the gateway reverses Alpha semantics; failure in both modes means the final upstream likely ignores mask guidance, so further work should use prompt hardening, a visual mask reference, or optional post-compositing instead of more encoding switches. Commit: `e5a2bc1`.
