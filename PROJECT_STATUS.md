...
## Current Snapshot
- Last Updated: 2026-07-21 17:05 +08:00
- Phase: v1.0.8 is locally integrated into `main`; the post-integration advanced-parameter deduplication is also committed and green, with user-side browser interaction remaining the only unautomated gate
...
- Current Focus: masked turns keep visible `maskSnapshot` review composites and hidden lossless `maskFileSnapshot` Alpha files so refresh-safe regenerate remains explicit. Per-turn deletion removes only conversation/session attachments and preserves generated history/output images. The final UI follow-up now gives each commonly used generation control one entry point: GPT size/custom size/quality/count and Banana count/aspect/resolution remain in the composer and no longer repeat inside Advanced Parameters.
- Latest Verification: zero unresolved P0/P1 findings. The final merged `main` passes 161/161 Node tests, 244/244 Python tests, 33/33 release-cache tests, `npm run test:size`, warning-free TypeScript/Vite production build, runtime imports, four-module `py_compile`, `git diff --check`, dependency/release-path scans and ledger health. A clean main-path smoke on temporary `14262` returned 200 for `/`, `/classic`, sessions, history and OpenAPI; health reported version `1.0.8` and main instance `9000fd5cac619dc5070d`, then the temporary PID was stopped. Live `14261` was restarted from main and now serves `index-D_52QQcP.js` from PID `30812`; `14260` remains untouched at PID `56600`. Legacy config forms, legacy session revisions/meta, legacy history list/delete behavior, conversation merge, queue/session boundaries and chat-reference isolation are covered by the passing compatibility regressions. Automated in-app browser access remains blocked by enterprise loopback policy and was not bypassed. Pre-existing untracked `PRODUCT.md` and `.impeccable/` remain untouched.
