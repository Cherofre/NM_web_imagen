...
- Latest Verification: 172/172 Node tests, 251/251 Python tests, Python compilation, Vite/TypeScript build, Rust fmt/check/check-locked, Rust unit tests, and PyInstaller backend rebuild passed. The notification regression fix, test-notification action, and capability permission compile successfully. A final Tauri release rebuild after these latest changes is pending because the currently running test EXE holds the release backend file.
...
- Residual Risks: direct uninstalled `target\release` or development EXEs can show `PowerShell` as the Windows notification source because Windows has no registered AppUserModel identity; the packaged installed Setup should show `NM Image Studio`. EXE/installer Authenticode status is `NotSigned`; a real old-version install/upgrade and clean-machine no-WebView2 acceptance remain untested; changing the output root requires restarting the desktop app; portable smoke needs a clean single-instance run; tray/notification behavior needs one manual Windows acceptance pass; `npm audit` still reports five build-chain findings for a later Vite/lockfile upgrade.
