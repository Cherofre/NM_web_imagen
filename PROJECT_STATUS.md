...
- Phase: v1.1.0 desktop updater implementation and local packaging verification
...
- Current Focus: keep the updater-enabled v1.1.0 artifacts local for manual UI testing; do not publish until the user authorizes a Release and a real old-version upgrade gate
...
- Latest Verification: 171/171 Node tests, Studio size rules, Python compilation, Vite production build, Rust `cargo check` with the local rsproxy override, real signed feed smoke, Tauri release EXE build, signed NSIS Setup/Updater artifact creation, portable ZIP/signature/feed generation, no-BOM feed and SHA256 checks, one-shell/one-backend startup, authenticated desktop sidecar health evidence in the backend log, and duplicate-launch single-instance smoke all passed. The unauthenticated health probe correctly returned 401.
- Artifacts: local updater staging contains `NM-Image-Studio-v1.1.0-Setup-x64.exe` (26.31 MB), `NM-Image-Studio-v1.1.0-Updater-x64.exe` plus `.sig`, `NM-Image-Studio-v1.1.0-Portable-x64.zip` (36.41 MB) plus `.sig`, `latest.json`, `portable-latest.json`, manifest and SHA256 sidecars. The staging directory is ignored and is not committed.
- Residual Risks: no GitHub Release or real old-version install/upgrade was performed; current GitHub `latest.json` is unavailable until publication, so the running EXE's About-page check is expected to show a clear network/feed failure. Portable replacement remains manual and automatic rollback helper does not exist. Missing-WebView2 behavior remains untested on a machine without the runtime; single-instance window lookup still relies on the unique `NM Image Studio` title; `npm audit` still reports five build-chain findings that require a later Vite major upgrade.
