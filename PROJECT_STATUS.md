...
- Residual Risks: direct uninstalled `target\release` or development EXEs can show `PowerShell` as the Windows notification source because Windows has no registered AppUserModel identity; the packaged installed Setup should show `NM Image Studio`. EXE/installer Authenticode status is `NotSigned`; a real old-version install/upgrade and clean-machine no-WebView2 acceptance remain untested; the latest migration/storage fixes still need a rebuilt EXE and one manual Windows acceptance; first-launch preferences are not implemented yet; portable smoke needs a clean single-instance run; tray/notification behavior needs one manual Windows acceptance pass; `npm audit` still reports five build-chain findings for a later Vite/lockfile upgrade.
