...
## Current Snapshot
- Last Updated: 2026-07-21 11:43 +08:00
- Phase: v1.0.8 compatibility-mode manual acceptance also failed; strict local restoration of every unpainted pixel is implemented and one real strict masked edit remains required before merge
...
- Current Focus: the 10:48 compatibility-mode result used the previous generated image as its base and correctly recorded a red external-background mask covering about 39.7%, yet protected-person drift still averaged 13.57 RGB levels versus 5.91 in the painted background, about 2.3 times more. The prompt was also targetless (`让那个区域换一种风格`). This closes the Alpha-direction investigation: upstream mask behavior cannot be trusted to protect the subject. Commit `ccc6fdc` now hardens concrete masked prompts, disables upstream prompt enhancement, and composites the original base back over every unpainted pixel before saving. Upstream controls only the painted region; local code guarantees the protected region.
- Latest Verification: final code passes 151/151 Node tests, 242/242 Python tests including 33 release-cache gates, `npm run test:size`, warning-free TypeScript/Vite build, four-module `py_compile`, `git diff --check`, portable-runtime rebuild with Pillow 12.2.0, and live HTTP smoke for `/`, `/api/health`, `index-D8_S6x_5.js` and `index-DFqFlLMm.css` on `14261`. Two targetless prompts return 400 before any upstream request. PID `37812` runs the current portable Python and instance ID. No paid strict-mask generation has been run, so painted-region usefulness and edge appearance still require manual acceptance. The separate `14260` service and pre-existing untracked `PRODUCT.md`, `.impeccable/`, and `config.local.json.bak` remain untouched.

## v1.0.8 Strict Mask Preservation
- Guarantee: after GPT returns each image, Pillow composites the first base image back through the mask's protected Alpha. Standard and compatibility encodings both resolve to the same user-painted red selection, so every unpainted pixel is restored locally before the PNG is saved, added to history or returned to Studio.
- Prompt guard: targetless instructions such as `遮罩部分换一下` or `换成另一种风格` are rejected in the browser and backend before upstream billing. Accepted mask prompts receive a local-only preservation preamble, and `enhance_prompt` is disabled so a gateway cannot broaden the edit through prompt rewriting.
- Visibility: current attachments show `严格遮罩` / `兼容 · 严格`; successful result metadata shows `严格保护`. History metadata records `mask_encoding` and `strict_mask` for later diagnosis.
- Resource boundary: strict masks support up to 8,294,400 pixels, matching the 4K product ceiling. The offline runtime gains Pillow 12.2.0 through a bundled 7.1 MB cp312 wheel; `start_web.ps1 -PrepareOnly` rebuilt and imported it successfully.
- Remaining limit: this guarantees preservation only outside the painted region. The model may still fail to make a useful change inside it, and anti-aliased boundaries may show a slight seam. A visual red-overlay reference is the next fallback only if a concrete prompt still leaves the painted region unchanged.
...
- Result: the 10:48 compatibility-mode generation still primarily changed the protected person. Keep the selector only as an explicit gateway diagnostic; `ccc6fdc` supersedes Alpha direction as the containment mechanism through strict local restoration.
...
- Implemented resolution: both Alpha directions drifted, so `ccc6fdc` now rejects targetless prompts, adds explicit preservation constraints and restores the original base outside the painted region before saving. Exact protection no longer depends on upstream mask adherence.
...
- Compatibility: `/api/studio/sessions` gains one optional additive `maskSnapshot` field per turn. Existing sessions without it, `/api/history`, generation multipart, queue behavior, config, Classic and old records remain compatible. Old records cannot reconstruct unsaved masks.
