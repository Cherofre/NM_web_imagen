...
## Current Snapshot
- Last Updated: 2026-07-21 16:48 +08:00
- Phase: v1.0.8 full pre-merge review is green after correcting the stale 1.0.7 version marker; local fast-forward into main is ready, with browser interaction remaining the only unautomated gate
...
- Current Focus: masked turns now keep two separate session attachments: visible `maskSnapshot` for the compact red review composite and hidden `maskFileSnapshot` for the exact lossless Alpha PNG. Both use existing session-reference normalization and canonical URL adoption. Regenerate still prefers the detached runtime payload, but after refresh it can reload the persisted Alpha, encoding and coverage, bind them to the reloaded first base and explicitly submit the mask. Completed turns also expose a neutral delete icon that becomes dangerous on hover/focus; confirmation explains that history/output images remain, while active queued/running turns are blocked.
- Latest Verification: the fresh whole-branch pass has zero unresolved P0/P1 findings. One merge blocker was found and fixed: code/spec were v1.0.8 while `VERSION`, README cache URL and the release gate still said 1.0.7, which could let a stale 1.0.7 backend survive startup reuse and discard new Alpha fields. After the 1.0.8 correction, 160/160 Node tests, 244/244 Python tests, 33/33 release-cache tests, `npm run test:size`, warning-free TypeScript/Vite build, runtime imports, four-module `py_compile`, `git diff --check`, dependency and forbidden-path scans all pass. Live `14261` returns 200 for `/`, `/classic`, `/api/health`, `/api/studio/sessions`, `/api/history`, `/openapi.json`, diagnostics, `index-jekAejtM.js` and `index-B-pQphnX.css`; health reports 1.0.8 and the served JS contains mask persistence and per-turn deletion. Automated browser access remains blocked by enterprise loopback policy and was not bypassed. Pre-existing untracked `PRODUCT.md`, `.impeccable/`, and `config.local.json.bak` remain untouched.
...
- Prompt behavior: targetless instructions such as `遮罩部分换一下` or `换成另一种风格` are rejected before upstream billing. Accepted prompts describe the complete result, identify red as a non-output selection marker, preserve unpainted content as far as the model can, and tell the model to blend only the immediate boundary instead of cutting or pasting. `enhance_prompt` remains disabled for masks.
