...
- Current Focus: the latest preview-workflow request is addressed at `c16881c`; ask the user to refresh `http://127.0.0.1:14260/` and verify that the preview header's `编辑遮罩` button promotes the displayed image to the first reference and opens the mask editor immediately, while the prior brush cursor, compact guidance and preview-control fixes still behave correctly. Do not package, write G:, call paid upstream APIs, push, merge, or publish before manual acceptance.
- Latest Verification: Python remains 238/238 from the unchanged backend matrix; 16 Node modules now pass 124/124; release/cache gates pass 33/33; `npm run test:size`; TypeScript/Vite production build; and live health/index/current JS/current CSS HTTP smoke pass. Current Studio assets are `index-veN3aNhZ.js` and `index-LnFPJluk.css`. The latest RED reproduced the missing preview-to-mask path; GREEN adds a visible loading-aware `编辑遮罩` action, safely converts same-origin outputs or existing reference previews to a `File`, switches to GPT generation, promotes that file to reference position one, invalidates only an unrelated old mask, and cancels stale loads when the preview closes or changes. Temporary manual service PID `56600` remains on port `14260`. No v1.0.7 package or G: sync exists. Only the pre-existing untracked `PRODUCT.md` and `.impeccable/` remain outside commits.
