...
- Latest Verification: Python compilation, security-boundary regression (67 tests), Vite/TypeScript production build, Studio size-rule check, 9 desktop Node tests, Rust fmt/check-locked and 2 Rust unit tests passed. The full prior 172/172 Node and 251/251 Python matrix remains the baseline; a final Tauri release rebuild after these latest changes is pending because the currently running test EXE holds the release backend file.
...
- Residual Risks: direct uninstalled `target\release` or development EXEs can show `PowerShell` as the Windows notification source because Windows has no registered AppUserModel identity; the packaged installed Setup should show `NM Image Studio`. EXE/installer Authenticode status is `NotSigned`; a real old-version install/upgrade and clean-machine no-WebView2 acceptance remain untested; the onboarding wizard, migration, and storage fixes still need a rebuilt EXE and one manual Windows acceptance; portable smoke needs a clean single-instance run; tray/notification behavior needs one manual Windows acceptance pass; `npm audit` still reports five build-chain findings for a later Vite/lockfile upgrade.
