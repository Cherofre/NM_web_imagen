...
- Latest Verification: 171/171 Node tests, 251/251 Python tests, Python compilation, Vite/TypeScript build, Rust fmt/check/check-locked and one Rust migration unit test, PyInstaller backend rebuild, and Tauri release build passed. The new portable smoke was blocked by an already-running installed NM Image Studio instance and must be rerun with that instance closed.
...
- Residual Risks: EXE/installer Authenticode status is `NotSigned`; a real old-version install/upgrade and clean-machine no-WebView2 acceptance remain untested; changing the output root requires restarting the desktop app; portable smoke needs a clean single-instance run; `npm audit` still reports five build-chain findings for a later Vite/lockfile upgrade.
