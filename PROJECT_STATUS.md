...
## Current Snapshot
- Last Updated: 2026-07-21 16:03 +08:00
- Phase: v1.0.8 now persists exact Alpha masks for refresh-safe regeneration and supports guarded per-turn deletion; browser interaction and one natural masked result remain the manual gate before merge
...
- Latest Verification: RED first reproduced missing Alpha persistence, canonical URL adoption, single-turn attachment pruning, refresh restoration, turn-only deletion and queue boundaries. GREEN passes 160/160 Node tests and 244/244 Python tests, `npm run test:size`, warning-free TypeScript/Vite build, four-module `py_compile`, and `git diff --check`. Live `14261` smoke returns 200 for `/`, `/api/health`, `/api/studio/sessions`, `index-jekAejtM.js` and `index-B-pQphnX.css`; the served JS contains `maskFileSnapshot` and the Chinese per-turn deletion confirmation. PID `53128` runs the current portable Python with the absolute worktree `app.py`; the separate `14260` listener remains PID `56600`. Automated browser interaction remains blocked by enterprise loopback policy, so refresh-safe mask regenerate, deletion confirmation/UI placement, Cancel, full-image editor fit and natural mask behavior remain manual checks. Pre-existing untracked `PRODUCT.md`, `.impeccable/`, and `config.local.json.bak` remain untouched.
