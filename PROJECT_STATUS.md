...
- Latest Verification: Python compilation, security-boundary regression (67 tests), Vite/TypeScript production build, Studio size-rule check, 59 desktop/UI Node tests, Rust fmt/check-locked and 2 Rust unit tests passed. After closing the running desktop/backend processes, the updated `npm run desktop:build` completed successfully on 2026-08-27; the follow-up build with the migration warning style also completed successfully, and fresh release artifacts are `studio-web/src-tauri/target/release/nm-image-studio-desktop.exe` (12.46 MB) and `studio-web/src-tauri/backend/nm-image-studio-backend.exe` (10.62 MB). The full prior 172/172 Node and 251/251 Python matrix remains the baseline.
