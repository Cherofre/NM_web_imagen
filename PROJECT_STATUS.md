...
## Current Snapshot
- Last Updated: 2026-07-21 15:28 +08:00
- Phase: v1.0.8 model-guided masks now support truthful current-page regenerate reuse and real stage-based whole-image fitting; browser interaction and one natural masked result remain the manual gate before merge
...
- Current Focus: every masked turn created in the current page now owns a detached runtime payload containing only the mask PNG, compact preview, coverage and encoding. Regenerate redownloads that turn's first base reference, binds the same mask payload to the new `File`, passes it explicitly through `SubmitOverrides`, reuses the saved mask-review snapshot and stores the payload on the new turn. If the page was refreshed or the first base cannot be loaded, the action stops with a clear message instead of silently generating without a mask. The mask editor now measures `.mask-editor-stage.clientWidth/clientHeight`, computes a separate fit scale, updates through `ResizeObserver`, and renders all canvases at the fitted dimensions; user zoom remains relative, and `0`/适配 returns to the full-image view.
- Latest Verification: TDD red reproduced all four missing behaviors before implementation. The revised code passes 156/156 Node tests and 242/242 Python tests including 33 release-cache gates, `npm run test:size`, warning-free TypeScript/Vite build, four-module `py_compile`, and `git diff --check`. Live `14261` smoke returns 200 for `/`, `/api/health`, `index-Ddq3RQ_S.js` and `index-DAW6P8i1.css`; the served JS contains the explicit current-page mask-unavailable warning. PID `27732` runs the current portable Python with an absolute `app.py` command line. Automated browser interaction remains blocked by enterprise loopback policy, so Cancel, regenerate-mask reuse, full-image editor fit and natural mask behavior remain manual checks. The separate `14260` service and pre-existing untracked `PRODUCT.md`, `.impeccable/`, and `config.local.json.bak` remain untouched.

## v1.0.8 Mask Regenerate and Editor Fit Follow-up
- Regenerate contract: runtime reuse is keyed by turn ID and deliberately excludes the original full base image. A current-page masked turn can recreate a valid attachment against its downloaded first reference and produces a new turn with the same visible mask snapshot. A refreshed/older turn retains only its review composite, which is not enough to reconstruct exact Alpha; the action therefore blocks and asks for a redraw.
