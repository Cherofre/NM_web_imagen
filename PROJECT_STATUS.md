...
- Branch: `main` at `52d447a`, pushed to `origin/main`; annotated tag `v1.1.0` and GitHub Release are live
...
- Current Focus: monitor v1.1.0 user feedback; the G: desktop distribution now has Chinese user-facing names and an extracted portable folder with a one-click launcher; retain the remaining Authenticode and real 1.1.0 → 1.1.1 follow-ups for v1.1.1
...
- Latest Verification: 171/171 Node tests, 251/251 Python tests, Python compilation, `pip check`, Vite/TypeScript build, Rust fmt/check, real signed feed smoke, normal and offline NSIS extraction smoke, portable startup smoke, packaged web runtime smoke, web/desktop coexistence, no-BOM feed and SHA256 checks, one-shell/one-backend startup, authenticated desktop sidecar health evidence, duplicate-launch single-instance smoke, and local release preflight all passed.
...
- Residual Risks: EXE/installer Authenticode status is `NotSigned`; a real old-version install/upgrade and clean-machine no-WebView2 acceptance remain untested; portable replacement remains manual and automatic rollback helper does not exist; `npm audit` still reports five build-chain findings for a later Vite/lockfile upgrade.
