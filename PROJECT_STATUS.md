...
- Phase: v1.1.1 published to GitHub and synchronized to both approved G: distribution roots
- Branch: `main` at `dca0422` with tag `v1.1.1`; local feature branch remains available; unrelated `.impeccable/` and `PRODUCT.md` stay untracked
...
- Current Focus: monitor v1.1.1 user acceptance. The published v1.1.0/G: distribution remains intact alongside the new v1.1.1 artifacts. Version metadata, package names, smoke defaults and release assertions target 1.1.1.
- Implemented: desktop settings/shortcuts/path actions/window state, a compact gear-only settings action beside the language switcher with `Ctrl+,`, a centered modal settings dialog with backdrop-click and `Esc` dismissal plus focus restoration, persistent customizable desktop shortcuts with conflict detection, per-shortcut clear and restore-all controls, an About-page updater-planning note, a skippable first-launch preference wizard for language, output location, notifications, and close behavior, default no-console launch, opt-in startup backend console, settings-launched live backend debug console, Windows DPAPI protection with plaintext-config migration, `gpt-5.6` migration, Windows named-mutex single instance with existing-window activation, a `KILL_ON_JOB_CLOSE` Job Object around the FastAPI sidecar plus debug console, portable-mode data routing, portable ZIP/manifest/SHA256 packaging, a pre-window WebView2 Evergreen registry check with a clear Chinese error dialog, the existing web black-and-white triangle mark reused for Tauri icons, a desktop migration scanner/importer with automatic target backup, compatibility for both `output` and `outputs` folder names, bounded nested-folder discovery for migration sources, a modern Explorer-style Windows folder picker using `IFileDialog`, folder-picker output sanitization for appended host diagnostics, a persisted configurable `IMAGE_TOOL_OUTPUTS_ROOT` that can point to Documents or another folder, runtime output-root switching without restarting the desktop shell, unified storage action button styling, clearer native command errors in storage/migration settings, and a restrained warning style for the destructive migration action
...
- Build Evidence: Tauri shell 8.14 MiB; backend directory 58 MiB / 708 files; normal Setup 26.30 MB; offline-WebView2 Setup 242.02 MB; portable desktop ZIP 36.41 MB; web ZIP 29.24 MB. Web release ZIP SHA256 (`-Web-x64`): `63cd8f8736f835c89ba07d01af41c9dda579fc3832cc9096cdda716c794d877f`.
- Latest Verification (2026-08-29 publication): full frontend regression passed 174/174 Node tests; version assertion passed; Tauri release build completed; portable desktop, web and both installer artifacts were rebuilt; package verification, local preflight, web package smoke, portable desktop smoke and web/desktop coexistence smoke all passed. Both G: destinations passed full `release_preflight.ps1 -ExpectedVersion 1.1.1`; their web ZIPs match the local SHA256 `93e38a58c0d60980e8c7bc5d0505495c75fb93cbd6630a7e01644dbbf94ec67f`. Current Studio assets are `index-DuGnlPAS.js` and `index-DbwxfS_c.css`.
- Artifacts: GitHub Release assets include the web ZIP, normal/offline installers, portable ZIP and signature, updater EXE and signature, and both update feeds. Desktop assets are also under `NM Image Studio Desktop` in both approved G: roots.
- Residual Risks: direct uninstalled `target\release` or development EXEs can show `PowerShell` as the Windows notification source because Windows has no registered AppUserModel identity; the packaged installed Setup should show `NM Image Studio`. EXE/installer Authenticode status is `NotSigned`; a real old-version install/upgrade and clean-machine no-WebView2 acceptance remain untested; the rebuilt EXE still needs one manual Windows acceptance for onboarding, migration, storage switching, portable single-instance behavior, tray, notifications, and the download/Save As folder action. Installer extraction smoke was not run because this machine already has an NM Image Studio installation and the script intentionally refuses to disturb it. The updater feed signing smoke remains unrun because no private key is configured. `npm audit` still reports five build-chain findings for a later Vite/lockfile upgrade. One packaging invocation through `npm run desktop:package` exposed a Windows PowerShell command-resolution issue for `Get-FileHash`; invoking the same package script directly with `powershell.exe -File ... -SkipBuild` succeeded, so the release artifacts were regenerated and verified through the direct script path.
- Distribution: `https://github.com/Cherofre/NM_web_imagen/releases/tag/v1.1.1`; four main packages plus manifests/SHA256 files are uploaded. The v1.1.1 web package and the Chinese-named desktop distribution are synchronized and passed destination preflight in both G: roots. The v1.1.0 release and assets remain available.
- Latest Download Fix Verification: after closing the running desktop/backend pair, `npm run desktop:build` completed successfully; the rebuilt EXE and backend were deployed to `I:\Program\NM Image Studio` with the previous pair backed up under `_backup-download-fix-20260827-144443`. Python compilation, the FastAPI attachment-header smoke check, and the 59 desktop/UI Node tests pass. The desktop app was left stopped for manual testing.
- Latest Native Download Verification: desktop build, Rust `cargo check --locked`, frontend build, and the 59 desktop/UI Node tests pass. Desktop output downloads now try the native save command, then an authenticated API/Blob fallback, and show a success toast. The deployed EXE hash is `3ED41F3E9341D248EB055B9C52D243A2C72F49131ECDFED41180A6E6F65AE11D`; the previous deployed pair is backed up under `I:\Program\NM Image Studio\_backup-download-fallback-20260827-151252`. The desktop app was left stopped for manual testing.
- Latest Save-As Verification: desktop now exposes native “另存为…” for output images, with “打开下载文件夹” in the image menu and the lightbox header. The release EXE/backend were rebuilt and deployed to `I:\Program\NM Image Studio`; the previous pair is backed up under `I:\Program\NM Image Studio\_backup-save-as-20260827-152957`. Rust/frontend builds and the 59 desktop/UI Node tests pass; native dialog interaction and a real saved-file smoke test remain for manual acceptance.
- Latest Download Toast Verification: the download success toast now contains a separate localized folder action, while the image “更多” menu only contains actions applicable to the image itself. The rebuilt shell/backend were deployed to `I:\Program\NM Image Studio`; the previous pair is backed up under `I:\Program\NM Image Studio\_backup-toast-download-20260827-154755`. The user-launched installed desktop instance was left untouched during the final audit.
- User-facing desktop distribution: `scripts/prepare_desktop_distribution.ps1` creates Chinese-named install/offline/portable files, extracts the portable ZIP, and adds `便携版\启动 NM Image Studio.bat`; the updater remains an internal component invoked by in-app About → Check for updates.
