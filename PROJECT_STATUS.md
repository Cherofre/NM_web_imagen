...
## Current Snapshot
- Last Updated: 2026-08-21 12:05 +08:00
- Phase: v1.1.0 desktop UX follow-up, application icon refresh, and installer build verification
...
- Implemented: desktop settings/shortcuts/path actions/window state, a persistent header `桌面设置` button with `Ctrl+,`, default no-console launch, opt-in startup backend console, settings-launched live backend debug console, Windows DPAPI protection with plaintext-config migration, `gpt-5.6` migration, Windows named-mutex single instance with existing-window activation, a `KILL_ON_JOB_CLOSE` Job Object around the FastAPI sidecar plus debug console, portable-mode data routing, portable ZIP/manifest/SHA256 packaging, a pre-window WebView2 Evergreen registry check with a clear Chinese error dialog, and a generated window/canvas + spark application icon wired into Tauri bundle config
...
- Latest Verification: Python 251/251, Node 170/170, Studio size rules, TypeScript/Vite production build, Python compilation, `cargo fmt --check`, `cargo check --offline`, and `npm run desktop:build` with the new icon set passed; prior Tauri NSIS normal/offline builds, installer extraction/uninstall smoke, portable ZIP manifest/startup smoke, web package smoke, web+desktop coexistence smoke, and user manual debug-console verification remain valid
