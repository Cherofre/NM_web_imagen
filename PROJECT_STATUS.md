...
## Current Snapshot
- Last Updated: 2026-07-21 10:26 +08:00
- Phase: v1.0.8 adds a reversible gateway mask-compatibility mode after repeated manual drift; automated verification is green and one real compatibility-mode generation remains required before merge
...
- Current Focus: repeated real requests confirm that the local editor selected the external background and sent an official transparent-selected PNG mask, yet the result changed the protected character 3.1–3.9 times more than the selected background. Commit `e5a2bc1` keeps that official `标准` mode as the default and adds an explicit `兼容网关` mode that reverses Alpha only for the current in-memory mask. This is a diagnostic/workaround for compatibility gateways that interpret opaque pixels as editable; it does not change backend routes, global config or old masks. A paid manual comparison is still required: success would identify reversed gateway semantics, while the same failure would strongly indicate that the final upstream ignores mask guidance.
- Latest Verification: final code passes 149/149 Node tests, `npm run test:size`, warning-free TypeScript/Vite build, 33/33 release-cache tests, `git diff --check`, and live HTTP smoke for `/`, `/api/health`, `index-DRPQN9Wy.js` and `index-DFqFlLMm.css` on `14261`. The backend was not changed, so the prior 239/239 Python baseline and `app.py` compile remain applicable rather than being rerun. Manual evidence still rejects reliable mask containment until `兼容网关` is tested with a concrete background replacement prompt. History/preview interaction fixes also await final manual confirmation. The separate `14260` service and pre-existing untracked `PRODUCT.md`, `.impeccable/`, and `config.local.json.bak` remain untouched.

## v1.0.8 Mask Gateway Compatibility Follow-up
...
- Acceptance signal: if compatibility mode changes the painted background while preserving the character better, keep the gateway workaround. If it still changes the character, treat the final upstream as mask-insensitive and move to explicit prompt hardening, a visual mask reference, or optional post-compositing rather than adding more Alpha toggles.
...
- Implemented diagnostic slice: preserve the official mask as `标准` and add a per-mask reverse-Alpha `兼容网关` option. Test that mode before adding prompt transformation. If both modes drift, automatically wrapping masked-edit prompts with explicit preservation constraints and warning on targetless prompts such as `遮罩部分换一下` is the next small option; exact outside-mask preservation still requires a separate post-composite design.
