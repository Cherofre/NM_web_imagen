...
## Current Snapshot
- Last Updated: 2026-07-20 17:09 +08:00
...
- Current Focus: new masked generation turns now carry a longest-edge 384px composite review snapshot and show a labeled `本次遮罩` thumbnail beside their reference images. The full-resolution editable mask remains queue-only; the lightweight review image is persisted through the existing Studio session-reference storage. History detail records whether it came from the sidebar, compact history or full browser. The foreground image preview now actively takes focus, Escape closes only the visible top layer, and its action order is `查看详情 -> 编辑遮罩 -> 作为参考图 -> 下载 -> 关闭`. The compact recent-history popover ignores scroll events originating inside its own grid, while page/viewport scroll and resize still dismiss it. Old turns cannot recover masks that were never saved. Manual rendering/click acceptance remains pending.
- Latest Verification: 147/147 Node tests, 239/239 Python tests, `npm run test:size`, warning-free TypeScript/Vite production build, 33/33 release-cache tests, `app.py` compile, `git diff --check`, ledger health and HTTP static smoke passed. After explicit user authorization, old PID `50968` was precisely verified and stopped, then current-branch Uvicorn started as PID `48232` on `14261`. Health returns 200/`ok=true`; `/assets/index-hlp9jyMM.js` and `/assets/index-CEN8B5E_.css` return 200. The current backend code is loaded, and the latest follow-up is frontend-only so no second restart is required. Manual browser interaction remains pending because automatic loopback browser access is policy-blocked. The separate `14260` service and pre-existing untracked `PRODUCT.md`, `.impeccable/`, and `config.local.json.bak` remain untouched.
