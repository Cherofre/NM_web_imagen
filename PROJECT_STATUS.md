...
- Preview stacking: local preview key handling stops propagation and closes only the preview. The global Escape handler now respects action menu, preview, secondary drawer and history priority. Compact history stays mounted underneath its preview, and history-backed previews expose a direct `查看详情` action.
