...
- Phase: v1.1.1 local release candidate rebuilt and verified; publication not yet authorized
- Branch: `codex/desktop-distribution-cn` with uncommitted storage/migration changes; published `main` remains at `52d447a` with tag `v1.1.0`
...
- Current Focus: v1.1.1 release candidate on `codex/desktop-distribution-cn`; published v1.1.0/G: distribution remains unchanged until explicitly released. Version metadata, package names, smoke defaults and release assertions now target 1.1.1.
...
- Latest Verification (2026-08-28 v1.1.1 candidate): full frontend regression passed 174/174 Node tests; the version assertion passed; Tauri release build completed; portable desktop, web and both installer artifacts were rebuilt; package verification, `release_preflight.ps1 -ExpectedVersion 1.1.1 -LocalOnly`, web package smoke, portable desktop smoke and web/desktop coexistence smoke all passed. Current Studio assets are `index-DuGnlPAS.js` and `index-DbwxfS_c.css`. v1.1.1 artifact hashes: web `93e38a58c0d60980e8c7bc5d0505495c75fb93cbd6630a7e01644dbbf94ec67f`; Setup and offline/portable hashes are recorded in their sidecar `.sha256` files.
...
- Distribution: `https://github.com/Cherofre/NM_web_imagen/releases/tag/v1.1.0`; web and desktop packages are synchronized and passed destination `release_preflight.ps1` in both G: roots.
