...
## Current Snapshot
- Last Updated: 2026-07-23 11:09 +08:00
- Phase: a post-v1.0.8 mask-prompt usability fix is locally implemented and verified on `codex/soft-mask-prompt-guidance`; manual browser acceptance and integration choice remain pending
- Superpowers Phase: published v1.0.8 baseline -> official-behavior review -> RED/GREEN soft-guidance fix -> full local verification -> manual browser gate
...
- Current Focus: broad prompts stay non-blocking, but model-chosen object replacement now receives a stronger intent expansion: remove the selected original object completely and replace it with a visibly different, scene-appropriate object while preserving physical contact.
- Latest Verification: the first manual result at `2026-07-23 11:00` proved that submission/mask routing worked but `换一个物品` merely regenerated the windmill. History records `/v1/images/edits`, `mask_used=true`, `mask_guidance=visual-alpha`, while the prior explicit `改成另一个物品` result successfully produced an ice-cream cone; this isolated the failure to weak broad-prompt semantics rather than a lost mask. The follow-up passes 163/163 previously completed Node tests, 245/245 Python tests, four-module `py_compile` and `git diff --check`. `14261` was restarted on PID `24672`; `14260` remains untouched. New production assets remain `index-B2GPwNFn.js` and `index-BWB4gA76.css`. Automated in-app browser interaction and a second paid upstream result remain unclaimed. Pre-existing untracked `PRODUCT.md` and `.impeccable/` remain untouched.
...
- UI contract: while a mask is active, the reference row shows one neutral compact hint and the textarea uses a mask-specific placeholder. Broad prompts such as `涂红的区域换一个物品` change the hint to explain that the model will choose unspecified details; no toast, modal or second confirmation interrupts submission.
...
- Model-chosen object replacement: wording such as `换一个物品`, `换个东西`, `改成另一个物品` and equivalent English replacement requests receives an additional non-blocking instruction to identify and completely remove the original selected object, choose a category/silhouette that is visibly different, and preserve grasping, occlusion, lighting and contact. Color/material/style edits do not receive this object-removal instruction.
...
- Release boundary: this branch is local only. No package, G: synchronization, push, tag or Release is authorized for this follow-up yet.
