...
- Phase: v1.1.0 published and synchronized to both approved G: distribution roots
...
- Current Focus: continue v1.1.1 desktop usability work on `codex/desktop-distribution-cn`; the published v1.1.0/G: distribution remains unchanged until this branch is explicitly released. Latest fix uses a native Tauri save command for desktop output images; web mode retains the backend attachment URL.
...
- Latest Verification: Python compilation, security-boundary regression (67 tests), Vite/TypeScript production build, Studio size-rule check, 59 desktop/UI Node tests, Rust fmt/check-locked and 2 Rust unit tests passed. After closing the running desktop/backend processes, the folder-picker update passed offline Rust checks/tests and the follow-up `npm run desktop:build` completed successfully on 2026-08-27; fresh release artifacts are `studio-web/src-tauri/target/release/nm-image-studio-desktop.exe` (12.46 MB) and `studio-web/src-tauri/backend/nm-image-studio-backend.exe` (10.62 MB). The full prior 172/172 Node and 251/251 Python matrix remains the baseline.
