...
## Current Snapshot
- Last Updated: 2026-07-20 15:15 +08:00
...
- Current Focus: the history surface still needs manual acceptance. The reported 589px header retains stable semantic rows. Composer height remains a browser/device preference; its only visible resize control is the centered grip on the real composer boundary, with double-click or `Home` reset. A new manual report found occasional swallowed clicks and an I-beam-like cursor in the left session list. Session titles now opt out of text selection, their child text cannot intercept pointer targets, the open button's hit surface extends through the card padding/gap, the delete control stays above that surface, and the active session exposes `aria-current`. GPT chat presets and saved profiles remain unchanged. The user should refresh and recheck `http://127.0.0.1:14261/`; rendered/clicked acceptance remains pending and must not be claimed.
- Latest Verification: the frontend follow-up passed 140 Node tests, `npm run test:size`, warning-free TypeScript/Vite production build, `tests.test_release_cache_busting` 33/33, `app.py` compile, `git diff --check`, and HTTP static smoke on port `14261`. The served root and new assets returned 200; the JS contains session `aria-current` and no `composer-reset-button`, while the CSS contains the session selection guard and expanded hit target. The prior backend slice remains verified at 238 Python tests and was not modified by this follow-up. Current assets are `index-LGy_W0H4.js` and `index-BGgboWHF.css`. Automatic real-browser interaction remains blocked by enterprise loopback policy. Temporary branch service PID `50968` is listening on port `14261`; the existing main UI service on `14260` is separate. The pre-existing untracked `PRODUCT.md`, `.impeccable/`, and `config.local.json.bak` remain outside commits.
...
- Compact state: clicking `历史窗` opens a viewport-clamped fixed popover for the latest 12 image-bearing records. Each record uses one representative thumbnail, multi-image records show a count badge, and clicking opens that record's existing preview gallery without adding another persistence model.
