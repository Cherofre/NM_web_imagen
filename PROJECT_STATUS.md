...
## Current Snapshot
- Last Updated: 2026-07-20 17:31 +08:00
- Phase: v1.0.8 manual acceptance found mask-guidance drift; history fixes remain locally verified, mask behavior requires a follow-up before merge
...
- Product Commits: `25b2915` (base refactor), `312a8d9` (recent-record popover), `1d9d1f5` (sidebar batch previews), `8a972c4` (responsive header/model controls), `2d8a263` (compact header grid and explicit GPT-5.6 tiers), `ac3926c` (stable narrow header plus persisted composer height), `41a34a5` (composer-edge resize affordance), `6af8700` (stable session-card hit targets), `41f0b2d` (mask review snapshots plus source-aware history navigation), and `7a6c528` (preview focus/action order plus quick-history internal-scroll guard)
...
- Current Focus: the saved mask review confirms the user painted the external background, not the character. Frontend export fills the PNG opaque and punches the painted overlay transparent, and the backend forwards that file as `mask` to `/v1/images/edits`, matching the official GPT Image mask direction. However the generated result changed the protected character much more than the selected background. GPT Image masks are documented as prompt-based guidance rather than hard pixel boundaries, the submitted prompt was only `遮罩部分换一下。`, and the configured `ai-gateway.local` compatibility layer has not been independently proven to preserve mask behavior. The candidate must not be merged as a reliable mask implementation until prompt hardening and/or strict compositing is decided and re-tested.
- Latest Verification: automated behavior remains green at 147/147 Node tests, 239/239 Python tests, `npm run test:size`, warning-free TypeScript/Vite production build, 33/33 release-cache tests, `app.py` compile, `git diff --check`, ledger health and HTTP static smoke. Manual evidence rejected the mask-behavior expectation: the red selected area covers about 42.8% of the saved review image, but base/result mean RGB difference was about 4.5 inside that selected background versus 16.1 in the protected character region. History/preview interaction fixes are still awaiting final manual confirmation. PID `48232` remains healthy on `14261`; the separate `14260` service and pre-existing untracked `PRODUCT.md`, `.impeccable/`, and `config.local.json.bak` remain untouched.
...
- Recommended next slice: automatically wrap masked-edit prompts with explicit `only change the selected region / preserve face, hair, clothing, pose, composition and unselected pixels` constraints and reject or warn on targetless prompts such as `遮罩部分换一下`. If exact outside-mask preservation is required, design an optional post-composite path that restores original pixels outside the selected region; this is a larger change because the release currently has no raster compositor dependency.
