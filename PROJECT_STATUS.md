...
## Current Snapshot
- Last Updated: 2026-07-22 14:46 +08:00
...
- Current Focus: mask prompts now follow the ChatGPT-style division of responsibility: the mask supplies edit location, while the user describes the desired change without repeating mask keywords. Broad but meaningful prompts remain valid and receive neutral, non-blocking guidance.
- Latest Verification: RED first reproduced the old frontend/backend rejection. GREEN passes 163/163 Node tests, 244/244 Python tests, four-module `py_compile`, `npm run test:size`, warning-free TypeScript/Vite production build and `git diff --check`. New production assets are `index-B2GPwNFn.js` and `index-BWB4gA76.css`. `14261` was restarted from this branch on PID `60920` and serves those assets; `14260` remains untouched. Legacy config, history, sessions, queue, regenerate, persisted masks and release-cache behavior remain covered. Automated in-app browser interaction remains unclaimed. Pre-existing untracked `PRODUCT.md` and `.impeccable/` remain untouched.
