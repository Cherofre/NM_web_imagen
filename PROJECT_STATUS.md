...
- Phase: v1.1.0 pre-release audit completed; local candidate artifacts remain unpublished
...
- Current Focus: preserve the locally verified candidate while closing updater metadata, Authenticode, clean-staging, and real 1.1.0 → 1.1.1 upgrade gates; do not publish until the user authorizes a Release
...
- Build Evidence: Tauri shell 8.14 MiB; backend directory 58 MiB / 708 files; normal Setup 26.30 MB; offline-WebView2 Setup 242.02 MB; portable desktop ZIP 36.41 MB; web ZIP 29.24 MB. The exact package hashes and full matrix are recorded in `docs/NM-Image-Studio-v1.1.0-pre-release-audit.md`.
...
- Artifacts: local updater staging contains current 1.1.0 Setup, signed updater EXE plus `.sig`, offline WebView2 Setup, signed portable ZIP plus `.sig`, web ZIP, `latest.json`, `portable-latest.json`, manifests and SHA256 sidecars. Older `v1.1.0-alpha.1` portable sidecars remain in the staging directory and must not be uploaded.
- Residual Risks: no GitHub Release or real old-version install/upgrade was performed; EXE/installer Authenticode status is `NotSigned`; updater packaging can leave a stale Setup manifest unless the normal installer script runs after updater packaging; `release_preflight.ps1` still expects the legacy parent-directory web ZIP path; portable replacement remains manual and automatic rollback helper does not exist; missing-WebView2 behavior remains untested on a machine without the runtime; `npm audit` still reports five build-chain findings that require a later Vite major/lockfile upgrade.
