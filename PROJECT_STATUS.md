...
## Current Snapshot
- Last Updated: 2026-07-21 14:29 +08:00
- Phase: v1.0.8 hard post-compositing was manually rejected because it created visible cut edges; model-guided red-selection input is implemented and one real natural-blend masked edit remains required before merge
...
- Current Focus: the 14:07 strict result did replace the selected background, but local pixel restoration exposed the coarse mask contour as a visible cut around hair, dress, lighting and mist. The user correctly rejected this as a fake edit. Commit `8ff25c9` removes all result-stage compositing. Before upstream submission, the first edit image now carries the user's selection as a 58% red overlay while the real Alpha mask is still sent separately; the prompt states that red is guidance only, asks for a complete final image, and explicitly permits a narrow natural transition instead of cutting or pasting along the contour. Concrete-target validation and disabled mask prompt enhancement remain.
- Latest Verification: the revised code passes 151/151 Node tests and 242/242 Python tests including 33 release-cache gates, `npm run test:size`, warning-free TypeScript/Vite build, four-module `py_compile`, and `git diff --check`. Live `14261` smoke returns 200 for `/`, `/api/health`, `index-kWbWZjat.js` and `index-DFqFlLMm.css`; the JS contains `遮罩引导` / `不做本地硬切拼接` and no `严格保护` badge. PID `62284` runs the current portable Python. Automated browser interaction remains blocked by enterprise loopback policy, and no paid post-change generation has been run, so natural blending and protected-area drift still require one manual single-image acceptance. The separate `14260` service and pre-existing untracked `PRODUCT.md`, `.impeccable/`, and `config.local.json.bak` remain untouched.
