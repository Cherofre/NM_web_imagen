...
- Phase: v1.0.7 Studio UI polish manual acceptance
...
- Current Focus: ask the user to refresh `http://127.0.0.1:14260/` and manually accept the polished Studio result cards, header menu, generation-settings popover, sidebar placement and narrow layout. Do not package, write G:, call paid upstream APIs, push, merge or publish before that acceptance.
- Latest Verification: product commit `4d22573` implements the approved five-part UI polish. Sixteen Node modules pass 128/128; `npm run test:size`, TypeScript/Vite production build and `tests.test_release_cache_busting` 33/33 pass. Current assets are `index-CZ6LzSZe.js` and `index-BsLUq92Z.css`; port `14260` returns v1.0.7 plus 200 for Studio, both current assets and `/classic`. Automatic real-browser interaction remains blocked by enterprise loopback policy, so rendered/clicked acceptance is intentionally not claimed. Temporary manual service PID `56600` remains available. No package or G: sync is authorized. Only the pre-existing untracked `PRODUCT.md` and `.impeccable/` remain outside commits.

## v1.0.7 Studio UI Polish Execution
- Results: one-image turns are expanded by default, use saved dimensions for the preview aspect ratio, preserve the whole composition with `contain`, and omit the redundant one-image toggle. Multi-image turns retain compact collapsed thumbnails and expose full controls after expansion.
- Image actions: `继续编辑`, `编辑遮罩` and `下载` remain directly visible; copy/apply/reference/open actions live in a keyboard-accessible details menu. The result mask action reuses the verified preview-to-mask path and does not alter the backend mask contract.
- Control hierarchy: top save was removed in favor of the existing drawer saves; `清空当前对话` moved into a dangerous header menu. Size, quality and count now share one generation-settings popover, with a small warning badge for multi-image counts.
- Layout and styling: sidebar count/list now share the flexible row, the duplicate new-chat action is gone, conversation and composer use the same 980px content column, ordinary controls use compact radii, switchers retain pills, the queue is neutral rather than glass/pulsing, and focus/press/reduced-motion states are strengthened.
