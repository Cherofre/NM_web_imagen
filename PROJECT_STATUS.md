...
- Current Focus: user manual feedback is addressed at `404bf55`; ask the user to refresh `http://127.0.0.1:14260/` and recheck mask shortcuts plus repeated clicks on the result-preview lower-right controls. Do not package, write G:, call paid upstream APIs, push, merge, or publish before manual acceptance.
- Latest Verification: Python remains 238/238 from the unchanged backend matrix; 16 Node modules now pass 121/121; release/cache gates pass 33/33; `npm run test:size`; TypeScript/Vite production build; and live health/index/current JS/current CSS HTTP smoke pass. Current Studio assets are `index-CwyU8QPK.js` and `index-DXsliuZ9.css`. Focused RED reproduced missing shortcuts and preview control isolation; a second RED captured fast double-click bubbling into canvas reset. GREEN removes duplicate mouse dragging, prevents controls from starting pointer capture or double-click reset, adds hover/active/focus feedback and live zoom percentage, and adds B/E/H, bracket, +/- and 0 shortcuts. Temporary manual service PID `56600` remains on port `14260`. No v1.0.7 package or G: sync exists. Only the pre-existing untracked `PRODUCT.md` and `.impeccable/` remain outside commits.
...
- v1.0.7 Frontend Mask Contract: native Canvas supports brush, eraser, move, size, undo/redo, clear/fill, zoom/fit, PNG export, first-base invalidation, exact in-memory `File` identity, queue snapshots, endpoint validation, focus containment, reduced-motion handling, bilingual copy, and discoverable B/E/H, `[ ]`, `- +`, `0` shortcuts. Base/mask badges are visible only in GPT generation mode so Banana/chat do not imply mask submission. Result-preview controls use pointer events only, ignore nested controls for drag/double-click reset, and provide subtle hover/press/focus feedback with a live percentage.
...
- Task 6 — user manual review found missing shortcuts and unreliable result-preview buttons; fixed at `404bf55`, with 121/121 Node and 33/33 release gates. Pending user re-acceptance after refresh. Automatic browser access remains policy-blocked; no v1.0.7 package, G: sync, push, merge, tag, PR, or release has occurred.
