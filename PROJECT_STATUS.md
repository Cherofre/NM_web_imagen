...
- Current Focus: publish the already verified `main` as GitHub Release `v1.0.8`. The final user-facing fixes add an explicit whole-session deletion warning and automatically bring an expanded multi-image result grid into view instead of leaving it above the viewport.
- Latest Verification: zero unresolved P0/P1 findings. The final one-click release path passes 163/163 Node tests, 244/244 Python tests, 33/33 release-cache tests, `npm run test:size`, warning-free TypeScript/Vite production build, ZIP extraction, portable-runtime startup smoke, local release preflight and both destination preflights. Production assets are `index-Dr9OIrAP.js` and `index-CEGbhIKr.css`. `I:\AI\Vibe Coding\NM_web_imagen-v1.0.8.zip` and both G: copies share SHA256 `ed171c9a1c74579a1a9375aa5e8b4fabf66a731b4764839c7b389d12a2f0046c`. Live `14261` remains on PID `30812`; `14260` remains untouched on PID `56600`; the temporary G: `7861` instance was stopped before final synchronization. Legacy config forms, legacy session revisions/meta, legacy history list/delete behavior, conversation merge, queue/session boundaries and chat-reference isolation remain covered. Automated in-app browser access remains blocked by enterprise loopback policy and was not bypassed. Pre-existing untracked `PRODUCT.md` and `.impeccable/` remain untouched.
