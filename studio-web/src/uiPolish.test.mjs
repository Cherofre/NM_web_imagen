...
test("queue entry floats as a glass capsule without pushing conversation layout", () => {
  ...
  assert.match(appSource, /<h3>任务队列<\/h3>/);
  assert.match(appSource, /function queueJobTitle\(job: QueueJob\)/);
  assert.match(appSource, /const jobTitle = queueJobTitle\(job\);/);
  assert.ok(appSource.includes('const jobMeta = [job.configName, job.model].filter(Boolean).join(" / ");'));
  assert.ok(appSource.includes('const jobElapsed = job.elapsedSeconds ? `${Math.round(job.elapsedSeconds)} 秒` : "";'));
  assert.match(appSource, /<strong>\{jobTitle\}<\/strong>/);
  assert.ok(appSource.includes('<span>{job.status === "queued" ? `${jobMeta} · 排队中` : jobElapsed ? `${jobMeta} · ${jobElapsed}` : jobMeta}</span>'));
  assert.match(appSource, /排队中，等待前面的生图任务完成/);
  assert.match(appSource, /compactInlineText\(job\.prompt, 24\) \|\| "未命名任务"/);
...
test("queue rows expose cancel retry apply and remove controls", () => {
  ...
  assert.match(appSource, /aria-label=\{`取消任务 \$\{job\.prompt \|\| "生成图片"\}`\}/);
  assert.match(appSource, /aria-label=\{`重试任务 \$\{job\.prompt \|\| "生成图片"\}`\}/);
  assert.match(appSource, /aria-label=\{`套用任务提示词 \$\{job\.prompt \|\| "生成图片"\}`\}/);
  assert.match(appSource, /aria-label=\{`移除任务 \$\{job\.prompt \|\| "生成图片"\}`\}/);
...
test("config drawer exposes a guarded delete-profile action", () => {
  ...
  assert.match(appSource, /aria-label=\{`删除配置 \$\{profile\.name \|\| "未命名配置"\}`\}/);
  assert.match(appSource, /title=\{canDeleteProfile \? "删除配置" : "至少保留一个配置"\}/);
...
test("config drawer exposes separate generation and chat diagnostics", () => {
  ...
  assert.match(appSource, /item\.capability === "generation" \? "生图" : "聊天"/);
...
test("image previews expose download and canvas zoom controls", () => {
  ...
  assert.match(appSource, /<div className="lightbox-zoom-tools" aria-label="图片缩放控制">/);
  assert.match(appSource, /aria-label="放大图片"/);
  assert.match(appSource, /aria-label="缩小图片"/);
  assert.match(appSource, /title="适配窗口"/);
  assert.match(appSource, /title="原始大小"/);
